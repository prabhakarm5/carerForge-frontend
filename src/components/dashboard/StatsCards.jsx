    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6">
      {cards.map((item, index) => {
        ...
        return (
          <div
            key={index}
            className="group bg-slate-900 border border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 hover:border-violet-600/50 transition-colors"
          >
            <div className="flex justify-between items-center sm:items-start gap-2">
              ...
              <div className={`p-2.5 sm:p-3 rounded-xl sm:rounded-2xl ${colors.bg} shrink-0 group-hover:scale-105 transition-transform`}>
                <Icon size={20} className={`${colors.text} sm:w-6 sm:h-6`} />
              </div>
            </div>
          </div>
        );
