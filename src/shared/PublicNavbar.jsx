...
const BASE_LINK =
  "inline-flex items-center rounded-lg px-2 py-1.5 text-[11px] font-bold whitespace-nowrap transition-colors duration-150 sm:rounded-xl sm:px-4 sm:py-2 sm:text-sm";
    ...
    <header className="fixed inset-x-0 top-0 z-[100] border-b border-white/10 bg-[#07060e]/95 backdrop-blur-md">
      ...
      >
        <Link
          ...
          className="min-w-0 shrink-0 transition-transform duration-150 hover:scale-[1.02] [&_.brand-logo-text]:hidden sm:[&_.brand-logo-text]:inline"
        >
          <BrandLogo size="sm" />
        </Link>

        <div className="flex min-w-0 shrink-0 items-center gap-1 sm:gap-2">
          {/* The logo already links home, so this label stays hidden on compact phones. */}
          <NavLink to="/" end className={({ isActive }) => `${navLinkClass({ isActive })} hidden sm:inline-flex`}>
            Home
          </NavLink>
